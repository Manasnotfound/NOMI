"""
NOMI v0.1 - Core Loop Prototype
Press button/Enter -> capture photo -> ask question -> Claude looks & answers -> speaks back

This is intentionally simple. Every "TODO" marks a spot you'll upgrade in later versions.
"""
...
# ---------------------------------------------------------------------------
# CONFIG - flip these as your hardware/skills grow
# ---------------------------------------------------------------------------
...
USE_PI_CAMERA_MODULE = False  # True if using ribbon-cable Pi Camera instead of USB webcam
...
def get_question() -> str:
    ...
    # --- Local voice transcription path (requires: pip install vosk) ---
    # Download a small model first: https://alphacephei.com/vosk/models
    # e.g. vosk-model-small-en-us-0.15, unzip it into ~/nomi/model
...
def ask_claude(image_path: str, question: str) -> str:
    """Sends the image + question to Claude and returns the answer text."""
...
def main():
    ...
    try:
        while True:
            ...
            answer = ask_claude(image_path, question)
            speak(answer)
